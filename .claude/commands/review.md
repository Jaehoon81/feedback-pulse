...
**호출 모드별 정책:**
- 사용자가 슬래시 커맨드로 직접 호출한 경우 (C-7-A 최종 검증) → **코드는 절대 수정하지 마라. 이 명령은 read-only 평가다.** 이슈는 보고만 하고 사용자가 fix 결정.
- `phase-review` step에서 sub-session으로 호출된 경우 (harness.md C 원칙 10번, E-3) → **이슈 발견 시 같은 step 안에서 즉시 fix까지 수행한다.** read-only 제약 해제.
...
```bash
git diff master...HEAD --name-only   # 변경된 파일 목록
git diff master...HEAD               # 변경 내용
```

현재 브랜치가 `master`이면 `git diff HEAD~1...HEAD`로 fallback. 우리 하네스는 `feat-{phase-name}` 브랜치를 만들므로 보통 `master` 대비 비교가 자연스럽다.

### 2-A. 하네스 자동 commit 식별 (phase 단위 review)

**호출 시점 2가지**:
- (자동) 각 phase 마지막 `phase-review` step이 sub-session으로 본 체크리스트를 자동 적용 (harness.md C 원칙 10번, E-3). 이슈 발견 시 같은 step에서 즉시 fix → 통과 시 다음 phase 자동 진입.
- (수동) C-7-A 최종 검증 — 6 Phase 완료 후 사용자가 전체 코드베이스 대상으로 `/review` 슬래시 커맨드 호출 (자동 review의 false negative 안전망).

하네스 `execute.py`는 step마다 `feat({phase}): step N — {name}` + `chore({phase}): step N output` 2단계 commit을 만든다. phase별로 묶어서 review하려면:

```bash
# 모든 하네스 step 커밋 (전체 phase) — feat = 일반 step, fix = phase-review step
git log master..HEAD --oneline --grep='^\(feat\|fix\)([^)]\+): step'

# 모든 하네스 메타 커밋 (output JSON 누적)
git log master..HEAD --oneline --grep='^chore([^)]\+): step'

# 특정 phase만 (예: 0-foundation) — feat/fix 모두 포착
git log master..HEAD --oneline --grep='^\(feat\|fix\)(0-foundation):'
git diff master..HEAD -- 'src/**' ':!phases/0-foundation/step*-output.json'
```
...
**호출 모드별 동작 분기 (L1 정책 재확인)**:
- **사용자 슬래시 커맨드 호출 (C-7-A 수동 검증)** → 위반 사항이 있으면 **파일 경로와 함께 수정 방안만** 제시하라. 코드를 직접 수정하지는 마라. H 우선순위 항목은 별도 fix commit (`fix:` conventional commits)으로 다음 작업의 첫 step에 포함한다.
- **phase-review step sub-session 호출 (harness.md C 원칙 10번, E-3)** → **이슈 발견 시 같은 step 안에서 즉시 fix 코드 작성**까지 수행하라. 수정 방안만 제시하고 멈추지 마라. fix 후 `npm run lint && npm run build && npm run test`로 재검증한다 (step.md 템플릿의 작업 #3~#4 참조).
