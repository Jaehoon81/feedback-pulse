...
> **결정 변경 (구현 단계)**: Recharts를 **도입하지 않고** `SentimentBar`를 순수 CSS 가로 막대로 구현했다(`src/components/SentimentBar.tsx` — `recharts` 의존성 미설치). MVP의 감성 분포는 단일 가로 막대 한 개라 라이브러리 번들 ~50KB가 과하고, CSS만으로 시맨틱 3색(green/red/neutral)을 충분히 표현할 수 있다. 향후 파이/도넛/트렌드 등 복합 차트가 실제로 필요해지는 시점에 Recharts 도입을 재검토한다. 이 결정 변경으로 ADR-015(차트 색상)도 "Recharts prop"이 아닌 "CSS 클래스" 기준으로 읽는다.
...
**결정**: 감성 분포 시각화 색상은 시맨틱 3색(긍정 `green-500`, 부정 `red-500`, 중립 `neutral-500`)만 사용한다. (ADR-006 결정 변경으로 Recharts 대신 CSS로 구현됐으므로, 색상은 prop이 아닌 Tailwind 클래스로 적용한다.)
