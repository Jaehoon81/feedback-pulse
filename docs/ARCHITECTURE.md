...
> **MVP 변형 (ADR-027)**: 와이어프레임의 모바일 햄버거 메뉴는 미구현. 모바일에서 사이드바는 main grid의 두 번째 행으로 자연 노출된다. 사이트 헤더는 좌측 로고(`feedback-pulse`) + 우측 `ThemeToggle`만 포함.
...
> **MVP 변형 (ADR-027)**: 와이어프레임의 헤더 통합(`[🍔 ← 홈 ⬇ 📋 복사 🌙]`)은 부분 변형. 사이트 헤더(`layout.tsx`)는 로고 + `ThemeToggle`만 두고, "← 홈" 버튼과 다운로드/복사 액션은 본문 상단의 `ReportActions` 컴포넌트로 분리되어 있다. 모바일 햄버거 + Drawer는 미구현.
...
- **데스크톱 (`≥1280px`)**: 우측 sticky anchor 네비 (옵션, IntersectionObserver 사용) — **MVP 미구현 (ADR-027)**, polish 단계 작업
- **첫 진입**: 영상 카드 + 6섹션이 순차 fade-in 0.4s × 0.05s stagger로 등장 (F-10) — MVP는 main 영역 단일 fade-in(0.4s)로 단순화, `prefers-reduced-motion` 가드 포함
