...
1. `review.md` 2-A의 phase 단위 패턴으로 직전 phase 변경 사항을 추출한다 (master는 본 프로젝트 기본 브랜치 가정 — 다른 환경은 `git rev-parse --abbrev-ref origin/HEAD`로 동적 조회):
   ```bash
   git diff master..HEAD -- 'src/**' ':!phases/2-analyzer/step*-output.json'
   git log master..HEAD --oneline --grep='^(feat|fix)(2-analyzer):'
   ```
...
- `tee .artifacts/reviews/...build.log` 리다이렉트 금지. 이유: `execute.py`가 phase 종료 시 자동 기록한다(`{date}-2-analyzer-build.log`). 사용자 수동 `/review` 호출(C-7-A) 시에만 review.md 3-7 `tee` 명령이 적용된다.
- git commit 메시지 직접 작성 금지. 이유: `execute.py`가 step 종료 시 `fix(2-analyzer): step 4 — phase-review` 형식으로 자동 commit한다.
